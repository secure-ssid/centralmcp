"""Unit tests for `mcp_servers.config.create_aaa_macauth_profile` and
...
def test_create_device_auth_profile_uses_v1alpha1_endpoint(monkeypatch, tool_name, resource):
    fake_client = FakeClient(FakeResponse(201, {"name": "centralmcp-lab"}))
    monkeypatch.setattr(config, "get_client", lambda: fake_client)
    tool = getattr(config, tool_name)

    result = tool("centralmcp-lab", body={"key": "value"})

    assert result["name"] == "centralmcp-lab"
    assert fake_client.calls == [
        (
            "POST",
            f"/network-config/v1alpha1/{resource}/centralmcp-lab",
            {"key": "value"},
        )
    ]
...
def test_create_device_auth_profile_dry_run_never_calls_client(monkeypatch, tool_name):
    ...
    tool = getattr(config, tool_name)
    result = tool("centralmcp-lab", dry_run=True)
...
def test_create_device_auth_profile_succeeds_on_clean_2xx(monkeypatch, tool_name):
    fake_client = FakeClient(FakeResponse(200, {"name": "centralmcp-lab"}))
    monkeypatch.setattr(config, "get_client", lambda: fake_client)
    tool = getattr(config, tool_name)
    result = tool("centralmcp-lab")
    assert result["name"] == "centralmcp-lab"
