...
from pipeline.stages import s6_configure
from pipeline.stages.s2_validate import (
    DEVICE_GROUP_PATHS,
...
MANIFEST = json.loads(
    (Path(__file__).resolve().parents[2] / "mcp_servers/openapi_gen/manifests/central.json").read_text()
)
...
class TestScopeResolution:
    ...
    def test_missing_group_degrades_to_global_only(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="pipeline.stages.s6_configure"):
