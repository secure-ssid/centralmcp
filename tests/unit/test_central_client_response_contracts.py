...
from pipeline.clients import central_client as cc
...
class TestDeprecationWarningDeduplication:
    def test_repeated_calls_warn_once(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="pipeline.clients.central_client"):
            for _ in range(5):
...
class TestWriteGateIndependence:
    ...
    def test_glp_transport_blocked_names_the_glp_flag(self, tmp_path, monkeypatch):
        ...
        assert "CENTRALMCP_GLP_V2BETA1_WRITES" in message
        assert "CENTRALMCP_CENTRAL_WRITES" not in message
