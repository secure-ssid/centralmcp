"""Unit tests for pipeline.release_restore -- safe restore and
smoke-validation of packaged release bundles.

Covers:
- Happy path: a real deterministic archive (built with
  ``pipeline.release_packaging``) restores, validates, and cleans up.
...
def real_bundle(tmp_path, validation_matrix_payload):
    ...
    packaged via pipeline.release_packaging into a real archive+checksum,
...
class TestHappyPath:
    ...
    def test_smoke_test_bundle_leaves_no_temp_directory_behind(self, real_bundle):
        archive_path, _checksum_path = real_bundle
        before = set(Path(tempfile.gettempdir()).glob("centralmcp-restore-smoke-*"))
        rr.smoke_test_bundle(archive_path)
        after = set(Path(tempfile.gettempdir()).glob("centralmcp-restore-smoke-*"))
        assert after == before
