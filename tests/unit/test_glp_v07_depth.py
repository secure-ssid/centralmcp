...
- Guarded writes in those families (VM power on/off + a bounded bulk
  ...
  behind the existing CENTRALMCP_GLP_V2BETA1_WRITES flag with dry_run/
  confirm.
...
- The pipeline.live_test_config / pipeline.artifact_contracts integration
  used by scripts/evaluate_glp_070_depth.py.

All host/region data is cross-checked against the committed manifest
(mcp_servers/openapi_gen/manifests/glp.json) so upstream drift fails loudly
