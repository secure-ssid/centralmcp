...
Covers ``mcp_servers.tool_router._bound_router_response`` both directly and
...
- Oversized list/dict responses get a stable ``_response_bounds``
  ...
  ``mcp_servers.shared.bound_collection_response``.
- A dict with nothing sliceable that still exceeds the byte budget falls
  back to a bounded text ``preview``.
- The two env var overrides (``CENTRALMCP_ROUTER_RESPONSE_MAX_ITEMS`` /
  ``CENTRALMCP_ROUTER_RESPONSE_MAX_BYTES``) are honored and invalid values
...
def _build_backend() -> FastMCP:
    srv = FastMCP("budget-test-backend")
