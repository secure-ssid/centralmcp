...
exit codes, and artifact redaction/leakage via pipeline.artifact_contracts.
"""
...
def _clean_env(monkeypatch):
    for var in (
        "CENTRALMCP_LIVE_TEST_GLP_READ",
        "CENTRALMCP_LIVE_TEST_GLP_WRITE",
...
def test_read_mode_blocked_by_default(tmp_path, capsys):
    ...
    assert "CENTRALMCP_LIVE_TEST_GLP_READ" in capsys.readouterr().err


def test_write_probe_requires_vm_id(monkeypatch, capsys):
    monkeypatch.setenv("CENTRALMCP_LIVE_TEST_GLP_READ", "1")
    monkeypatch.setenv("CENTRALMCP_LIVE_TEST_GLP_WRITE", "1")
