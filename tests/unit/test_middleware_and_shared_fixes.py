...
* device_type_for_troubleshoot must treat the generic ``SWITCH``/``SWITCHES``
  ...
  ``switch`` URL segment.
* The three local-index RAG tools carry READ_ONLY_LOCAL (openWorldHint=False).
"""
...
from mcp_servers._middleware.install import install_middleware
from mcp_servers._middleware.response_envelope import ResponseEnvelopeMiddleware
...
class TestOnErrorRoutedThroughAfter:
    def test_substitute_passes_through_after_call(self):
        ...
        result = asyncio.run(srv._tool_manager.call_tool("boom", {}))
...
# ---------------------------------------------------------------------------
# Fix 6: local RAG tools carry READ_ONLY_LOCAL (openWorldHint=False)
# ---------------------------------------------------------------------------
class TestRagLocalAnnotations:
    def test_read_only_local_profile(self):
        assert sh.READ_ONLY_LOCAL.readOnlyHint is True
        assert sh.READ_ONLY_LOCAL.destructiveHint is False
        assert sh.READ_ONLY_LOCAL.openWorldHint is False

    def test_local_rag_tools_are_read_only_local(self):
        import mcp_servers.rag as rag
