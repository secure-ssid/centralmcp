...
2. The global ``CENTRALMCP_READONLY`` kill switch hides/blocks only
...
from mcp_servers.shared import DESTRUCTIVE, DIAGNOSTIC, IDEMPOTENT_WRITE, READ_ONLY
...
def _build_backend() -> FastMCP:
    ...
    # Give the diagnostic tool the DIAGNOSTIC annotation (FastMCP's bare
...
class TestWrapperRateExemption:
    ...
    def test_wrapped_read_costs_exactly_one_token(self, wired_router, monkeypatch):
        from mcp_servers._middleware.rate_limit import RateLimitMiddleware
        ...
        async def scenario() -> int:
            ...
            await router._dispatch_tool(router.mcp.get_context(), "read_tool", {})
...
class TestReadonlyKillSwitch:
    ...
    def test_dispatch_blocks_write_and_destructive(self, wired_router, monkeypatch):
        ...
        for name in ("write_tool", "destroy_tool"):
            ...
            assert "CENTRALMCP_READONLY" in out.get("error", "")
...
# ---------------------------------------------------------------------------
# Fix 7 (standalone seam): install_platform_write_gate honors CENTRALMCP_READONLY
# ---------------------------------------------------------------------------
class _GateTool:
...
class TestStandaloneGateReadonly:
    def _backend(self):
        import mcp_servers.shared as sh

        return _GateMCP(
            "aruba-config",  # central: writes enabled by default
            {
    ...
    def _call(self, mcp_obj, name):
        return asyncio.run(mcp_obj._tool_manager.call_tool(name, {}))
    ...
    def test_readonly_blocks_writes_on_standalone_backend(self, monkeypatch):
        ...
        assert "CENTRALMCP_READONLY" in blocked.get("error", "")
    ...
    def test_router_server_is_not_gated(self):
        ...
        router_mcp = _GateMCP("aruba-tool-router", {})
