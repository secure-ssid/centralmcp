...
def _backend(payload_size: int = 3) -> FastMCP:
    srv = FastMCP("aruba-monitoring")
...
def wired(monkeypatch):
    backend = _backend()
    monkeypatch.setattr(router, "_BACKENDS", {"aruba-monitoring": "fake.mod"})
...
class TestBatchFailureContainment:
    ...
    def test_items_report_their_resolved_backend_server(self, wired):
        ...
        assert out["results"][0]["server"] == "aruba-monitoring"
...
class TestBatchObservability:
    def test_single_target_batch_resolves_the_real_tool(self, wired):
        ...
        assert (tool, backend, capability) == ("echo", "aruba-monitoring", "read")
    ...
    def test_multi_target_batch_uses_a_bounded_constant_label(self, wired):
        ...
        assert backend == "aruba-monitoring"
    ...
    def test_audit_middleware_accepts_the_batch_as_a_dispatching_tool(self):
        from mcp_servers._middleware.audit_log import _DISPATCHING_TOOL_NAMES
...
class TestStrictBudgetShrinkStages:
    ...
    def test_stage_one_shrinks_ok_results_last_first(self):
        items = [
            {"index": i, "id": str(i), "tool": "big_list", "server": "aruba-monitoring",
