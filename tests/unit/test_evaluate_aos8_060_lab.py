...
from pipeline.aos8_target_adapters import CandidateAction, Operation, TargetType

module = importlib.import_module("scripts.evaluate_aos8_060_lab")


def candidate(identifier: str = "centralmcp-lab-wlan") -> dict[str, Any]:
    return {
        "object_type": "wlan",
...
def test_execute_plan_requires_explicit_env_gate(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(PermissionError, match="CENTRALMCP_CENTRAL_WRITES=1"):
