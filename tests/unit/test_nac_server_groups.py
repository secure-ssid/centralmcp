...
`delete_server_group`) added to `mcp_servers.nac`.
...
def test_create_server_group_sends_expected_method_path_body(monkeypatch):
    fake_client = FakeClient(FakeResponse(201, {"name": "centralmcp-lab-sg"}))
    ...
    assert result["name"] == "centralmcp-lab-sg"
    assert fake_client.calls == [
        (
            "POST",
            "/network-config/v1alpha1/server-groups/centralmcp-lab-sg",
            {
...
def test_create_server_group_dry_run_previews_payload_and_skips_client():
    ...
    assert result["endpoint"] == "/network-config/v1alpha1/server-groups/centralmcp-lab-sg"
...
def test_create_server_group_dry_run_never_calls_client(monkeypatch):
    ...
    nac.create_server_group("centralmcp-lab-sg", ["radius-1"], dry_run=True)
...
def test_create_server_group_raises_on_non_2xx_response(monkeypatch):
    ...
    with pytest.raises(WriteResultError):
        nac.create_server_group("centralmcp-lab-sg", ["radius-1"])
...
def test_delete_server_group_sends_expected_method_and_path(monkeypatch):
    ...
    result = nac.delete_server_group("centralmcp-lab-sg")

    assert result["status_code"] == 204
    assert fake_client.calls == [
        ("DELETE", "/network-config/v1alpha1/server-groups/centralmcp-lab-sg", None)
    ]
...
def test_delete_server_group_dry_run_never_calls_client(monkeypatch):
    ...
    monkeypatch.setattr(nac, "get_client", fail_get_client)
    result = nac.delete_server_group("centralmcp-lab-sg", dry_run=True)
    assert result == {"dry_run": True, "name": "centralmcp-lab-sg"}
...
def test_delete_server_group_raises_on_non_2xx_response(monkeypatch):
    ...
    monkeypatch.setattr(nac, "get_client", lambda: fake_client)
    with pytest.raises(WriteResultError):
        nac.delete_server_group("centralmcp-lab-sg")
...
def test_get_server_group_sends_expected_path(monkeypatch):
    fake_client = FakeClient(FakeResponse(200), get_payload={"name": "centralmcp-lab-sg"})
    monkeypatch.setattr(nac, "get_client", lambda: fake_client)

    result = nac.get_server_group("centralmcp-lab-sg")

    assert result["name"] == "centralmcp-lab-sg"
    assert fake_client.calls == [
        ("GET", "/network-config/v1alpha1/server-groups/centralmcp-lab-sg", None)
    ]
