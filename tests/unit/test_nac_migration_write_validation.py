...
def _calls() -> list[tuple[str, WriteCall]]:
    return [
        ...
        (
            "delete_auth_server",
            lambda: nac.delete_auth_server("centralmcp-lab-radius"),
        ),
        (
            "create_aaa_profile",
            lambda: nac.create_aaa_profile("centralmcp-lab-aaa"),
        ),
        (
            "delete_aaa_profile",
            lambda: nac.delete_aaa_profile("centralmcp-lab-aaa"),
        ),
        (
            "create_server_group",
            lambda: nac.create_server_group(
                "centralmcp-lab-sg", ["centralmcp-lab-radius"]
            ),
        ),
        (
            "delete_server_group",
            lambda: nac.delete_server_group("centralmcp-lab-sg"),
        ),
    ]
