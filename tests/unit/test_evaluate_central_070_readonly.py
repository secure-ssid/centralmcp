...
def _clear_live_test_env(monkeypatch):
    for platform in ("CENTRAL",):
        monkeypatch.delenv(f"CENTRALMCP_LIVE_TEST_{platform}_READ", raising=False)
        monkeypatch.delenv(f"CENTRALMCP_LIVE_TEST_{platform}_WRITE", raising=False)
...
def test_live_read_runs_bounded_steps_when_enabled(monkeypatch, tmp_path):
    monkeypatch.setenv("CENTRALMCP_LIVE_TEST_CENTRAL_READ", "1")
