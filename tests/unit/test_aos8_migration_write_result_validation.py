...
Covers:
- mcp_servers.shared.validate_write_result as a standalone helper (raw HTTP
  response/status, response envelopes with errors/success/status fields,
  and NOT rejecting legitimate empty 2xx bodies).
- mcp_servers.aos8._aos8_migration_write_invoker's endpoint-invocation
...
from mcp_servers.shared import WriteResultError, validate_write_result
from pipeline.aos8_target_adapters import Operation
...
def test_write_invoker_tool_branch_fails_on_error_envelope(monkeypatch):
    ...
    import mcp_servers.config as config_tools
