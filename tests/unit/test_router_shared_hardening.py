...
def _gated_backend(name: str) -> FastMCP:
    srv = FastMCP(name)
...
def _call(server: FastMCP, name: str) -> Any:
    return asyncio.run(server._tool_manager.call_tool(name, {}))
...
class TestPlatformForServerName:
    ...
    @pytest.mark.parametrize("server_name", ["aruba-rag", "aruba-tool-router", "", None])
...
class TestStandaloneWriteGate:
    def test_destructive_tool_is_blocked_when_gate_closed(self, monkeypatch):
        ...
        server = _gated_backend("aruba-glp")
        ...
        assert "CENTRALMCP_GLP_V2BETA1_WRITES" in result["error"]
    ...
    def test_gate_is_per_platform_not_shared(self, monkeypatch):
        ...
        monkeypatch.delenv("CENTRALMCP_GLP_V2BETA1_WRITES", raising=False)
        glp = _gated_backend("aruba-glp")
        central = _gated_backend("aruba-config")
    ...
    def test_ungated_server_is_left_untouched(self):
        from mcp_servers.shared import _WRITE_GATE_INSTALLED_ATTR

        server = _gated_backend("aruba-rag")
    ...
    def test_composes_with_middleware_in_either_order(self, monkeypatch):
        from mcp_servers._middleware import NullStripMiddleware, install_middleware

        monkeypatch.delenv("CENTRALMCP_GLP_V2BETA1_WRITES", raising=False)

        gate_first = _gated_backend("aruba-glp")
        install_platform_write_gate(gate_first)
        install_middleware(gate_first, [NullStripMiddleware()])

        middleware_first = _gated_backend("aruba-glp")
    ...
    def test_blocked_result_is_convertible_for_structured_output(self, monkeypatch):
        """FastMCP calls call_tool(convert_result=True) on the MCP path; the
        ...
        converted = asyncio.run(
            server._tool_manager.call_tool("destroy_thing", {}, convert_result=True)
        )
    ...
    def test_allowed_call_still_converts_normally(self, monkeypatch):
        ...
        converted = asyncio.run(
            server._tool_manager.call_tool("read_thing", {}, convert_result=True)
        )
...
def _install_fake_backend(monkeypatch, backend: FastMCP, server_name: str) -> None:
...
class TestDirectRegistration:
    def test_disabled_writes_are_not_registered(self, monkeypatch):
        monkeypatch.delenv("CENTRALMCP_GLP_V2BETA1_WRITES", raising=False)
        backend = _gated_backend("aruba-glp")
        _install_fake_backend(monkeypatch, backend, "aruba-glp")
    ...
    def test_original_tool_object_is_published(self, monkeypatch):
        backend = FastMCP("aruba-monitoring")
        ...
        _install_fake_backend(monkeypatch, backend, "aruba-monitoring")
        target = FastMCP("router-direct")
    ...
    def test_existing_router_tool_names_are_not_overwritten(self, monkeypatch):
        backend = _gated_backend("aruba-monitoring")
        _install_fake_backend(monkeypatch, backend, "aruba-monitoring")
        target = FastMCP("router-direct")
...
class TestAtomicBackendLoad:
    ...
    def test_import_failure_does_not_stop_other_backends(self, monkeypatch):
        good = _gated_backend("aruba-monitoring")
        self._reset(monkeypatch, {"broken-core": "broken.mod", "aruba-monitoring": "good.mod"})
        ...
        assert router._tool_backend_names["read_thing"] == "aruba-monitoring"
    ...
    def test_duplicate_tool_names_raise_without_partial_state(self, monkeypatch):
        first = _gated_backend("aruba-monitoring")
        second = _gated_backend("aruba-config")
        self._reset(monkeypatch, {"aruba-monitoring": "a.mod", "aruba-config": "b.mod"})
        monkeypatch.setattr(
            router.importlib,
            "import_module",
