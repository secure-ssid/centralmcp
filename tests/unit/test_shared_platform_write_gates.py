"""Unit tests for the per-platform write-gate helpers in mcp_servers.shared.
...
class TestDefaults:
    ...
    def test_optional_products_default_to_shared_toggle(self, platform, monkeypatch):
        for gate in PLATFORM_WRITE_GATE_NAMES:
            monkeypatch.delenv(f"CENTRALMCP_{gate.upper()}_WRITES", raising=False)
        monkeypatch.delenv("CENTRALMCP_GLP_V2BETA1_WRITES", raising=False)
...
class TestOptionalBackendIntegration:
    @pytest.mark.parametrize(
        ...
        [
            ("mcp_servers.aos8", "aos8", "CENTRALMCP_AOS8_WRITES"),
            (
                "mcp_servers.edgeconnect",
                "edgeconnect",
                "CENTRALMCP_EDGECONNECT_WRITES",
            ),
            ("mcp_servers.apstra", "apstra", "CENTRALMCP_APSTRA_WRITES"),
