...
- Audit-log helpers hit ``/audit-log/v1/logs`` and
  ...
  manifest (``mcp_servers/openapi_gen/manifests/glp.json``), and passed
...
MANIFEST = json.loads(
    (Path(__file__).resolve().parents[2] / "mcp_servers/openapi_gen/manifests/glp.json").read_text()
)
...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("CENTRALMCP_CENTRAL_WRITES", raising=False)
    yield
    monkeypatch.delenv(_V2BETA1_WRITES_FLAG, raising=False)
...
class TestPollTask:
    ...
    def test_in_progress_then_success(self, monkeypatch):
        glp, inner = _client()
        monkeypatch.setattr("pipeline.clients.glp_client.time.sleep", lambda s: None)
    ...
    def test_unknown_status_warns_and_surfaces_in_timeout(self, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="pipeline.clients.glp_client"):
            with pytest.raises(RuntimeError) as exc:
...
class TestGlpGateIndependence:
    ...
    def test_gate_message_names_only_the_glp_flag(self):
        ...
        assert "CENTRALMCP_GLP_V2BETA1_WRITES=1" in message
        assert "independent of CENTRALMCP_CENTRAL_WRITES" in message
