...
Covers:
- Disabled by default (CENTRALMCP_METRICS unset) — every hook is a
...
from mcp_servers._middleware.metrics import metrics_enabled
...
class TestDisabledByDefault:
    def test_metrics_enabled_false_without_env(self, monkeypatch):
        monkeypatch.delenv("CENTRALMCP_METRICS", raising=False)
...
class TestRegistryAggregates:
    def test_record_call_counts_and_latency(self):
        ...
        assert bucket["backend"] == "aruba-ops"
    ...
    def test_concurrent_asyncio_calls_are_counted_exactly(self, monkeypatch):
        ...
        async def _run():
            await asyncio.gather(
                *[srv._tool_manager.call_tool("fast_tool", {"x": i}) for i in range(50)]
            )
