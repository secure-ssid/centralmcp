...
class TestGenerationAware401Collapse:
    def test_sync_concurrent_401s_on_shared_client_collapse_to_one_refresh(
        ...
        singleton from mcp_servers.shared.get_client()) both read the same
        ...
        monkeypatch.setattr("pipeline.clients.central_client.time.sleep", lambda s: None)
    ...
    def test_async_401_passes_generation_through(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("pipeline.clients.central_client.asyncio.sleep", _fake_async_sleep)
        ...
        monkeypatch.setattr("pipeline.clients.central_client.httpx.AsyncClient", FakeAsyncClient)
