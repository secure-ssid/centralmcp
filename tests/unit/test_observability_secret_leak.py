...
def _stack(tmp_path: Path, registry: MetricsRegistry) -> tuple[FastMCP, Path]:
    audit_file = tmp_path / "audit.jsonl"
    srv = FastMCP("secret-leak-test")
    install_middleware(
        srv,
        [
            NullStripMiddleware(),
...
class TestNestedPayloadNoLeak:
    def test_nested_secret_result_never_leaks(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CENTRALMCP_METRICS", "1")
        monkeypatch.setenv("CENTRALMCP_AUDIT_LOG", "1")
...
class TestFailurePathNoLeak:
    def test_exception_message_never_leaks(self, tmp_path, monkeypatch):
        ...
        # FastMCP wraps tool exceptions as ToolError(f"... {original}"), so
...
class TestConcurrentNoLeak:
    def test_concurrent_secret_bearing_calls_never_leak_or_miscount(
        ...
        srv = FastMCP("concurrent-secret-test")
            ...
            await asyncio.gather(
                *[
                    srv._tool_manager.call_tool(
                        "secret_tool", {"password": f"hunter2-secret-{i}"}
                    )
                    for i in range(25)
                ]
            )
...
# ---------------------------------------------------------------------------
# pipeline.artifact_contracts reuse: same guarantee for the v0.7 evidence
