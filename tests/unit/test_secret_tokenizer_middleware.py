...
to mcp_servers._middleware.install that it relies on.

Covers:
- Disabled by default (CENTRALMCP_TOKENIZE_SECRETS unset) — every hook is
  a strict no-op, byte-for-byte, so adopting this middleware is safe.
- Enabled: sensitive-keyed result values become opaque cmcp_secret_* tokens
...
from mcp_servers._middleware.secret_tokenizer import _TOKEN_RE, tokenize_enabled
...
class TestDisabledByDefault:
    def test_tokenize_enabled_false_without_env(self, monkeypatch):
        monkeypatch.delenv("CENTRALMCP_TOKENIZE_SECRETS", raising=False)
    ...
    def test_before_call_is_noop_when_disabled(self, monkeypatch):
        ...
        result = mw.before_call("set_webhook", {"webhookSecret": "cmcp_secret_" + "a" * 32})
...
class TestEnabledRoundTrip:
    def test_sensitive_value_is_tokenized_in_result(self, monkeypatch):
        monkeypatch.setenv("CENTRALMCP_TOKENIZE_SECRETS", "1")
        mw = SecretTokenizeMiddleware()
    ...
    def test_unknown_token_string_passed_through_unchanged(self, monkeypatch):
        ...
        result = mw.before_call("set_webhook", {"webhookSecret": "cmcp_secret_" + "0" * 32})
...
class TestInstallMiddlewareContextPassing:
    ...
    def test_context_aware_middleware_receives_context(self, monkeypatch):
        ...
        monkeypatch.setenv("CENTRALMCP_TOKENIZE_SECRETS", "1")
        srv = FastMCP("ctx-passing-secret-test")
