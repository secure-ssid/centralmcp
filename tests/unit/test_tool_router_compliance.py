...
``evaluate_compliance_policy`` in ``mcp_servers.tool_router``.
...
- Read-only annotation on the registered FastMCP tool.
"""
...
class TestRegistration:
    ...
    def test_annotated_read_only(self):
        ...
        assert tool.annotations.readOnlyHint is READ_ONLY.readOnlyHint
        assert tool.annotations.destructiveHint is READ_ONLY.destructiveHint
...
class TestEvaluateCompliancePolicyFailsClosed:
    ...
    def test_oversized_policy_fails_closed(self):
        fn = _evaluate_compliance_policy_fn()
        from pipeline import compliance as pc
...
# the registered router tool (never just pipeline.compliance directly).
# ---------------------------------------------------------------------------
