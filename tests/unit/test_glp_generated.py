"""Unit tests for the generated GreenLake (GLP) tools on mcp_servers.glp.

Covers: committed manifest count + provenance/digests, direct registration on
the curated aruba-glp server (without disturbing curated tools), representative
read/write dispatch, the GLP write gate (CENTRALMCP_GLP_V2BETA1_WRITES, fail
...
def _register_generated_glp_tools():
    ...
    try:
        ...
        if prev is None:
            os.environ.pop("CENTRALMCP_GLP_GENERATED_TOOLS", None)
        else:
            os.environ["CENTRALMCP_GLP_GENERATED_TOOLS"] = prev
...
def test_auth_isolation_uses_glp_not_central(monkeypatch):
    ...
    import mcp_servers.central_generated as cg
