...
``plan_reconciliation_schedule`` in ``mcp_servers.tool_router``.
...
- Both tools always produce a valid ``artifact`` (or an explicit
  ``artifact_error``) via ``pipeline.artifact_contracts`` -- never raising.
...
from mcp_servers.shared import DESTRUCTIVE, IDEMPOTENT_WRITE, READ_ONLY
...
def _build_planner_backend() -> FastMCP:
    srv = FastMCP("planner-test-backend")
