...
curated `aruba-config` / `aruba-monitoring` / `aruba-ops` tools, plus a
...
manifest (`mcp_servers/openapi_gen/manifests/central.json`, 1,678 operations)
...
(`CENTRALMCP_CENTRAL_WRITES`, default enabled — `enforce_platform_write`
in `mcp_servers/shared.py`), the executed response is validated
...
### `mcp_servers/config.py` (75 → 80 tools)
...
### `mcp_servers/monitoring.py` (85 → 87 tools)
...
### `mcp_servers/ops.py` (40 → 41 tools)
...
Credential-gated per `pipeline/live_test_config.py`; every artifact is
written through `pipeline/artifact_contracts.write_artifact`
...
- **`--live-read`:** swaps in the real `mcp_servers.shared.get_client()`
  and runs the same bounded steps as GET-only calls. Requires
  `CENTRALMCP_LIVE_TEST_CENTRAL_READ=1`; a GET-only guard is installed on
...
- **`--live-write` / `run_disposable_write_lifecycle`:** a disposable
  ...
  lab scope. Gated by `CENTRALMCP_LIVE_TEST_CENTRAL_WRITE=1` **and** the
...
`CENTRALMCP_LIVE_TEST_CENTRAL_READ` was **not set** in this environment, so
...
`pipeline/live_test_config.py`) — set
`CENTRALMCP_LIVE_TEST_CENTRAL_READ=1` explicitly to run the bounded
GET-only path.
