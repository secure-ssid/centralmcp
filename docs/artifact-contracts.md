...
[`scripts/evaluate_aos8_060_lab.py`](https://github.com/secure-ssid/centralmcp/blob/main/scripts/evaluate_aos8_060_lab.py)).
...
| `MIGRATION_REPORT_METADATA` | `MigrationReportMetadata` | Metadata *about* a migration report (counts, formats, hashed device references) -- never the raw per-device rows, which stay in `pipeline/reporter.py`'s CSV/JSON/HTML outputs. |
...
1. **Secrets** -- reuses `mcp_servers.shared.redact_sensitive` (not
...
`pipeline/aos8_migration_orchestrator.py`'s `MigrationRunStore`). It returns
...
`pipeline/live_test_config.py` is the generalized, reusable form of the
...
always-on MCP servers' write-tool gates (`CENTRALMCP_CENTRAL_WRITES`,
`CENTRALMCP_GLP_V2BETA1_WRITES`, `CENTRALMCP_PRODUCT_ACCESS`, documented in
...
| Read opt-in | `CENTRALMCP_LIVE_TEST_<PLATFORM>_READ=1` enables bounded, read-only live calls. |
| Disposable-write opt-in | `CENTRALMCP_LIVE_TEST_<PLATFORM>_WRITE=1` **and** the read opt-in together enable a disposable create/read-back/delete round trip against a lab-owned target. The write flag alone is never sufficient. |
...
Supported platform keys (shared with `mcp_servers.shared.PLATFORM_WRITE_GATE_NAMES`):
`central`, `glp`, `aos8`, `edgeconnect`, `apstra`, `mist`, `clearpass`,
`uxi`, `axis`.

| Platform | Read env var | Write env var | Credential env vars |
|---|---|---|---|
| Central | `CENTRALMCP_LIVE_TEST_CENTRAL_READ` | `CENTRALMCP_LIVE_TEST_CENTRAL_WRITE` | `SOURCE_BASE_URL`, `SOURCE_CLIENT_ID`, `SOURCE_CLIENT_SECRET` |
| GLP | `CENTRALMCP_LIVE_TEST_GLP_READ` | `CENTRALMCP_LIVE_TEST_GLP_WRITE` | `TARGET_BASE_URL`, `TARGET_CLIENT_ID`, `TARGET_CLIENT_SECRET` |
| AOS8 | `CENTRALMCP_LIVE_TEST_AOS8_READ` | `CENTRALMCP_LIVE_TEST_AOS8_WRITE` | `AOS8_BASE_URL`, `AOS8_USERNAME`, `AOS8_PASSWORD` |
| EdgeConnect | `CENTRALMCP_LIVE_TEST_EDGECONNECT_READ` | `CENTRALMCP_LIVE_TEST_EDGECONNECT_WRITE` | `EDGECONNECT_BASE_URL`, `EDGECONNECT_API_TOKEN` |
| Apstra | `CENTRALMCP_LIVE_TEST_APSTRA_READ` | `CENTRALMCP_LIVE_TEST_APSTRA_WRITE` | `APSTRA_BASE_URL`, `APSTRA_API_TOKEN` |
| Mist | `CENTRALMCP_LIVE_TEST_MIST_READ` | `CENTRALMCP_LIVE_TEST_MIST_WRITE` | `MIST_HOST`, `MIST_API_TOKEN` |
| ClearPass | `CENTRALMCP_LIVE_TEST_CLEARPASS_READ` | `CENTRALMCP_LIVE_TEST_CLEARPASS_WRITE` | `CLEARPASS_BASE_URL`, `CLEARPASS_API_TOKEN` |
| UXI | `CENTRALMCP_LIVE_TEST_UXI_READ` | `CENTRALMCP_LIVE_TEST_UXI_WRITE` | `UXI_CLIENT_ID`, `UXI_CLIENT_SECRET` |
| Axis | `CENTRALMCP_LIVE_TEST_AXIS_READ` | `CENTRALMCP_LIVE_TEST_AXIS_WRITE` | `AXIS_BASE_URL`, `AXIS_API_TOKEN` |

```python
from pipeline import live_test_config as live_test
...
uv run ruff check pipeline/artifact_contracts.py pipeline/live_test_config.py
```
