...
`mcp_servers/tool_router.py` is the recommended MCP entrypoint. Instead of
...
```json
{
  "platform": "central",
  "capability": "write",
  "gate": {"env_var": "CENTRALMCP_CENTRAL_WRITES", "state": "enabled", "source": "platform_default"},
...
Configure the two budgets with `CENTRALMCP_ROUTER_RESPONSE_MAX_ITEMS`
(default 200, range 1-200) and `CENTRALMCP_ROUTER_RESPONSE_MAX_BYTES`
...
- A server restart invalidates every outstanding cursor. A malformed,
  tampered, expired (`CENTRALMCP_ROUTER_CURSOR_TTL_SECONDS`, default 900s,
...
FastMCP tool manager, so arguments get FastMCP validation/coercion and the
...
Set `CENTRALMCP_READONLY=1` for a server-wide write kill switch, independent
...
```json
{
  "error": "Tool 'build_underlay_ssid' is disabled because CENTRALMCP_READONLY is set. Unset CENTRALMCP_READONLY to allow write/destructive tools.",
...
(`python -m mcp_servers.<x>`). A platform whose own gate is enabled is still
fully read-only while `CENTRALMCP_READONLY` is set.
</div>

<div class="docs-callout docs-callout--warning" markdown="1">
<h3>Per-platform write gates</h3>

Central defaults to writes **enabled** (`CENTRALMCP_CENTRAL_WRITES=0` opts
out); GLP defaults to writes **disabled** (`CENTRALMCP_GLP_V2BETA1_WRITES=1`
opts in). A blocked GLP write looks like this:

```json
{
  "error": "Tool 'invite_glp_user' is disabled because glp writes are not enabled. Set CENTRALMCP_GLP_V2BETA1_WRITES=1 to allow glp write workflows.",
  ...
  "execution_contract": {
    "platform": "glp",
    "capability": "write",
    "gate": {"env_var": "CENTRALMCP_GLP_V2BETA1_WRITES", "state": "disabled", "source": "platform_default"},
    "dry_run": {"supported": false, "state": "unsupported"},
    "confirm": {"supported": false, "required": false},
    "idempotent": true,
    "next_action": "Set CENTRALMCP_GLP_V2BETA1_WRITES=1, then retry only after explicit user approval."
  }
}
```
...
The seven optional starters (`clearpass`, `mist`, `apstra`, `aos8`,
`edgeconnect`, `uxi`, `axis`) share `CENTRALMCP_PRODUCT_ACCESS`, which
defaults to `read-only`. That hides optional write tools from `find_tool` and
blocks direct dispatch through `invoke_tool`. Set
`CENTRALMCP_PRODUCT_ACCESS=read-write` for lab workflows that need guarded
writes -- those write tools still default to `dry_run=True`. Use
`CENTRALMCP_<PLATFORM>_WRITES=1` (e.g. `CENTRALMCP_AXIS_WRITES=1`) for a
...
and RAG workflows. If `CENTRALMCP_ROUTER_MODE` is omitted, the router uses
...
| Default router | 16 client-visible tools[^compliance-tool] |
| Complete backend index | 6,700 tools |
| Direct-all router | 6,707 client-visible tools |

</div>

[^compliance-tool]: v0.7 added `plan_tool_workflow` and
    `plan_reconciliation_schedule`; the post-v0.7 compliance expansion adds
    `evaluate_compliance_policy`, raising the default-mode count to 16.
    `minimal` mode remains the same three-tool surface.

The complete catalog spans nine platform surfaces plus RAG: nine generated
manifests contain 6,143 reproducible operations (6,126 register as active
generated tools; 574 curated tools bring the executable backend total to
...
Optional products can be enabled either by `CENTRALMCP_TOOLSETS` or by
`CENTRALMCP_PRODUCTS`; see [optional-products.md](optional-products.md) for
...
other HTTP path here. With only `CENTRALMCP_METRICS_HTTP=1` set, the route
...
(<code>pipeline/compliance.py</code>) over caller-supplied
...
ready for `pipeline.artifact_contracts.write_artifact`; none of the three
