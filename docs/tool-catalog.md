...
centralmcp registers **6,700 backend tools** when every generated surface and
guarded write is enabled. Direct-all router mode adds seven router-native tools for
**6,707 total**. The recommended minimal router exposes only `find_tool`,
...
Adding 574 curated tools yields the 6,700 executable backend total. Capability
totals across the full catalog are 3,148 read, 165 diagnostic, 2,544 write, and
...
| `aruba-monitoring` | 73 | 88 | Health, inventory, topology, BSSID mappings, applications, onboarding, AP tunnels, config health, notification rules, guarded Central GET, and reports |
...
| `direct` + all toolsets/products | 6,707 | Full schema introspection and debugging |
