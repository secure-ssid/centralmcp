...
- `pipeline/artifact_contracts.py` defines eight schema-versioned artifact
  ...
  writes it.
- `pipeline/live_test_config.py` generalizes the AOS8 0.6 lab-evaluation
...
gated behind `pipeline/live_test_config.py` and were not run live in this
...
`pipeline/aos8_rollback.py` adds reverse-dependency-order rollback/
...
`aruba-rag` adds four bounded, read-only tools (573 curated backend tools
...
- **Response budgets**: every `invoke_tool`/`invoke_read_tool` result now
  ...
  (`CENTRALMCP_ROUTER_RESPONSE_MAX_ITEMS`, default 200;
  `CENTRALMCP_ROUTER_RESPONSE_MAX_BYTES`, default 200,000). An in-budget
...
- **Opt-in redacted audit log** (`CENTRALMCP_AUDIT_LOG=1`): one JSONL record
...
- **Opt-in bounded in-process metrics** (`CENTRALMCP_METRICS=1`, optional
  `CENTRALMCP_METRICS_HTTP=1` for a `GET /metrics` snapshot on streamable
  ...
  exception messages.
- A shared `classify_outcome` helper (`mcp_servers/_middleware/_outcome.py`)
...
- **Release bundle packaging** (`scripts/build_release_bundle.py`)
  ...
  (`pipeline/sbom.py`) from `uv.lock`; a `CHECKSUMS.txt`; a
...
- Optional product writes remain hidden and blocked in read-only mode
  (`CENTRALMCP_PRODUCT_ACCESS=read-only` default).
...
2. Rebuild the router tool index for your desired profile:
   ...
   CENTRALMCP_PRODUCT_ACCESS=read-write CENTRALMCP_GLP_GENERATED_TOOLS=1 \
     uv run python scripts/ingest_tools.py --products all
   ```
...
4. No credential, environment-variable, or config-file schema changes are
   required. New env vars (`CENTRALMCP_AUDIT_LOG`, `CENTRALMCP_METRICS`,
   `CENTRALMCP_METRICS_HTTP`, `CENTRALMCP_LIVE_TEST_<PLATFORM>_READ`/
...
uv run python scripts/restore_release_bundle.py dist/centralmcp-release-artifacts-v0.7.0.tar.gz
```

No live vendor API calls were made anywhere in this validation pass.
`CENTRALMCP_LIVE_TEST_<PLATFORM>_READ`/`_WRITE` were left unset throughout, so
...
Release bundle `centralmcp-release-artifacts-v0.7.0.tar.gz` (built locally,
...
`centralmcp-rag-index-v0.7.0.tar.gz` (51,737 prose chunks / 244 specs).
