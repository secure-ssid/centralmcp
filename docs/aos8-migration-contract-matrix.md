...
2. **PARTIALLY RESOLVED — WLAN security normalization.** `parse_wlans`
   ...
   (`NewCentralAdapter._map_wlan`, `pipeline/aos8_target_adapters.py`,
...
`pipeline/aos8_target_adapters.py`, added by the `aos8-verification` todo after
...
| **Classification** | `unsupported` | **conditional** — the object contract (create/update/delete/read) is verified and tested for the simple (no dot1x/mac/server-group refs) subset (`test_simple_aaa_profile_maps_only_verified_fields`, `test_aaa_profile_rejects_ap_persona_and_exposes_update_delete_operations`), but every such candidate stays `blocked` (never `ready`) because the SHARED config-assignment profile-type `"aaa-profile"` is, like `"auth-servers"`/`"dot1xauth"`/`"macauth"`/`"server-groups"`, only inferred by endpoint-path convention and not independently evidenced (`_unverified_assignment_blocker`, §2.1); any profile carrying real 802.1X/MAC/server-group references is `unsupported` today |
...
| **Classification** | `unsupported` | **conditional** — the bare-name object contract is verified and tested (`test_bare_dot1x_and_macauth_profiles_map_on_gateway_switch_only`), but the candidate stays `blocked` (never `ready`) pending the same unverified `"dot1xauth"` config-assignment profile-type as §6.3 (§2.1); any profile with additional settings is `unsupported` (`test_rich_dot1x_and_macauth_profiles_are_rejected_not_guessed`) |
...
| **Target method/path** | none verified | `NewCentralAdapter._map_mac_auth_profile` → `POST/PATCH/DELETE/GET /network-config/v1alpha1/macauth/{name}` (`pipeline/aos8_target_adapters.py:1702-1707`, shared `_map_device_auth_profile` helper) — same bare-name-only contract as §6.4 |
| **Schema context** | n/a | `x-supportedDeviceType: [Gateway, Switch CX, Switch PVOS]` — same Gateway/switch-only caveat as §6.4 |
| **Classification** | `unsupported` | **conditional** — same bare-name-verified, config-assignment-`blocked` status as §6.4, pending the unverified `"macauth"` profile-type (§2.1) |
...
| **Classification (RADIUS)** | `unsupported` | **conditional** — the object contract (create/update/delete/read, secret handling) is verified and tested (`test_radius_requires_caller_secret_and_masks_preview`, `test_radius_auth_server_has_verified_update_and_delete_operations`), but every candidate stays `blocked` (never `ready`) because the SHARED config-assignment profile-type `"auth-servers"` is not independently evidenced (same §2.1 gap as §6.3–§6.5/§6.8) |
...
| **Target method/path** | none verified | `NewCentralAdapter._map_server_group` → `POST/PATCH/DELETE/GET /network-config/v1alpha1/server-groups/{name}` (`pipeline/aos8_target_adapters.py:1709-1851`) — builds the ordered `servers` array from resolved same-type `auth_server` dependencies |
...
3. ~~Implement New Central adapter mappers for `dot1x_auth_profile`, `mac_auth_profile`, `server_group` (ordered `servers`, type-aware)~~ — **done** (§6.4/§6.5/§6.8); every one of them, plus the pre-existing `auth_server`/`aaa_profile` mappers, stays `blocked` (never a complete migration) pending the §2.1/§5 config-assignment profile-type confirmation, not pending further adapter code. Still to implement, in dependency order: `policy`→Gateway-policy-or-CX-role-ACL (persona-branched), `route` (static-route, Gateway/AP), `vrrp`/`vrrp-interface` (with explicit VLAN-interface attachment), `ap_group`.
