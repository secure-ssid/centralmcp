...
| EdgeConnect | status, guarded GET/write, appliance inventory/system/alarm/interface/disk/overlay/topology/tunnel, route-map, tunnel-metadata, and VRF-segment reads | `EDGECONNECT_BASE_URL`, `EDGECONNECT_API_TOKEN`, optional `EDGECONNECT_AUTH_HEADER` | Read/write starter; writes dry-run by default |
