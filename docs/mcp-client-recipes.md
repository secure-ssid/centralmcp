...
demand. The complete index can contain 6,700 backend tools, while minimal
...
Edit `.mcp.json` and replace `/path/to/centralmcp` with your local clone
path:
...
```json
{
  "mcpServers": {
    "aruba-tool-router": {
      "command": "/path/to/centralmcp/.venv/bin/python3",
      "args": ["/path/to/centralmcp/mcp_servers/tool_router.py"],
      "cwd": "/path/to/centralmcp",
      "env": {
        "PYTHONPATH": "/path/to/centralmcp",
...
```json
{
  "mcpServers": {
    "aruba-tool-router": {
      ...
      "args": ["${workspaceFolder}/mcp_servers/tool_router.py"],
      "cwd": "${workspaceFolder}",
      "env": {
...
registers the six core Aruba servers (`aruba-monitoring`, `aruba-config`,
`aruba-ops`, `aruba-nac`, `aruba-glp`, `aruba-rag`) plus
`aruba-central-generated` directly, so it costs more tool-list context than
...
Keep the `aruba-tool-router` server entry enabled for normal use:
...
```json
{
  "name": "aruba-tool-router MCP server (minimal)",
  "runtimeExecutable": "python",
  "runtimeArgs": ["-m", "mcp_servers.tool_router"],
  "env": {
    "CENTRALMCP_ROUTER_MODE": "minimal",
    "CENTRALMCP_TOOLSETS": "central,glp,rag"
  }
...
(`aruba-monitoring`, `aruba-config`, `aruba-ops`, `aruba-nac`, `aruba-glp`)
...
startup banner prints the selected products and `CENTRALMCP_PRODUCT_ACCESS`
