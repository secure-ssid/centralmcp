...
repository's layering convention keeps ``pipeline/*.py`` free of
``scripts.*`` imports (only ``scripts/*.py`` imports other
``scripts/*.py`` and ``pipeline/*.py``, never the reverse).
...
``pipeline.artifact_contracts.write_artifact`` (canonical, sorted-key JSON)
...
artifact kind in ``pipeline/artifact_contracts.py``.
...
BUNDLE_NAME_PREFIX = "centralmcp-release-artifacts"
...
def build_provenance_manifest(
    *,
    ...
    return {
        "provenance_schema": "centralmcp-release-provenance-v1",
