...
def build(specs_dir: Path = SPECS_DIR, db_path: Path = DB_PATH) -> dict[str, int]:
    ...
    Builds into a sibling temp file and atomically os.replace()s it over the
    live path only after the full build commits — an interrupted build leaves
    the previous good index in place instead of a valid-but-empty DB that
    lookup_api would silently serve [] from.
    """
