...
- Mandatory recursive redaction before writing: secrets are stripped by
  :func:`mcp_servers.shared.redact_sensitive` (reused, not reimplemented)
...
- Deterministic content hashing and an atomic (temp-file + ``os.replace``)
  ...
  ``pipeline/aos8_migration_orchestrator.py``'s ``MigrationRunStore``.
...
3. ``migration_report_metadata`` -- metadata *about* a migration report
   ...
   report rows themselves (those stay in ``pipeline/reporter.py``'s
...
9. ``validation_matrix_result`` -- a bounded, per-category snapshot of the
   ...
   (see ``pipeline/validation_matrix.py``). Never a record of an executed
...
``dataclasses``, matching the rest of the pipeline/mcp_servers code) and
...
# `validation_matrix_result` (see pipeline/validation_matrix.py):
#
...
#   explicitly set the platform's read opt-in (pipeline/live_test_config.py)
...
# ---------------------------------------------------------------------------
# Recursive redaction -- reuses mcp_servers.shared.redact_sensitive for
...
def redact_artifact_payload(
    ...
    *,
    ...
    shared, independently-tested ``mcp_servers.shared.redact_sensitive``.
...
class MigrationReportMetadata:
    ...
    The raw per-device rows stay in ``pipeline/reporter.py``'s CSV/JSON/HTML
