...
built through :mod:`pipeline.artifact_contracts` (never a hand-rolled JSON
shape) and gated through :mod:`pipeline.live_test_config` (never an ad hoc
env var convention).
...
2. **Live read evidence** (opt-in only): when
   ...
   :data:`pipeline.artifact_contracts.LIVE_LIFECYCLE_EVIDENCE` artifact.
   ...
   disabled for every platform (see ``pipeline/live_test_config.py``), and
...
from mcp_servers.openapi_gen.manifest import load_manifest, manifest_path
...
def _status_step(platform: str) -> dict[str, Any]:
    ...
    """
    try:
        module = importlib.import_module(f"mcp_servers.{platform}")
        if platform == "axis":
