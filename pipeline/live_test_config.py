...
- Read-only live calls require an explicit, separate opt-in
  (``CENTRALMCP_LIVE_TEST_<PLATFORM>_READ=1``).
- Disposable-write live calls (a bounded create/read-back/delete round trip
  ...
  explicit write opt-in (``CENTRALMCP_LIVE_TEST_<PLATFORM>_WRITE=1``).
...
``mcp_servers.shared`` per-platform *write-tool* gates
(``CENTRALMCP_CENTRAL_WRITES``, ``CENTRALMCP_GLP_V2BETA1_WRITES``,
``CENTRALMCP_PRODUCT_ACCESS``, etc.), which control whether the always-on
...
from mcp_servers.shared import PLATFORM_WRITE_GATE_NAMES
...
# (mcp_servers.shared.PLATFORM_WRITE_GATE_NAMES) so every v0.7 harness
...
def live_test_read_env_var(platform: str) -> str:
    ...
    return f"CENTRALMCP_LIVE_TEST_{_validate_platform(platform).upper()}_READ"


def live_test_write_env_var(platform: str) -> str:
    """Return the disposable-write opt-in env var name for ``platform``."""
    return f"CENTRALMCP_LIVE_TEST_{_validate_platform(platform).upper()}_WRITE"
