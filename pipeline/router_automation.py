...
This module backs two `mcp_servers.tool_router` read-only tools --
`plan_tool_workflow` (dependency ordering) and `plan_reconciliation_schedule`
(recurring reconciliation planning) -- but never imports FastMCP or any
...
`pipeline.artifact_contracts.write_artifact`.
"""
...
# with, pipeline.artifact_contracts.MAX_ROUTER_RECONCILIATION_EXCLUDED so a
...
def partition_reconciliation_candidates(
    ...
    *,
    ...
    ``pipeline.artifact_contracts.MAX_ROUTER_RECONCILIATION_EXCLUDED``).
...
# pipeline.artifact_contracts.build_artifact/write_artifact. Kept here
