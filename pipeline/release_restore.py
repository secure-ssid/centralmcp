...
* post-extraction schema validation of every contract-typed JSON file
  ...
  :func:`pipeline.artifact_contracts.build_artifact`), plus basic
...
def validate_extracted_bundle(root: Path) -> tuple[tuple[str, ...], tuple[str, ...]]:
    ...
    :func:`pipeline.artifact_contracts.build_artifact` for that entry's
...
def _temporary_restore_dir() -> Iterator[Path]:
    with tempfile.TemporaryDirectory(prefix="centralmcp-restore-smoke-") as tmp:
        yield Path(tmp)
