...
posture recorded throughout `pipeline/aos8_target_adapters.py` and
`pipeline/aos8_migration_orchestrator.py`. It never invents a new inverse
...
- ``execute_rollback_plan`` mirrors
  ...
  ``write_invoker`` already enforces (e.g. ``CENTRALMCP_CENTRAL_WRITES``).
...
No network calls; no dependency on `mcp_servers/` (same convention as
every other `pipeline/aos8_*.py` module).
"""
...
# (`CENTRALMCP_CENTRAL_WRITES`, `CENTRALMCP_AOS8_WRITES`, ...): rollback is a
...
ROLLBACK_WRITE_GATE_ENV_VAR = "CENTRALMCP_AOS8_ROLLBACK_WRITES"
...
def rollback_writes_enabled() -> bool:
    ...
    ordinary migration-apply writes (e.g. ``CENTRALMCP_CENTRAL_WRITES``) --
...
def plan_rollback(
    ...
    Args:
        applied_candidates: candidate dicts (the same shape produced by
            `pipeline.aos8_migration.build_migration_plan` and persisted by
            `pipeline.aos8_migration_orchestrator.MigrationRunStore`) for
