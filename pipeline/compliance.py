...
``mcp_servers.tool_router``, but never imports FastMCP, ``mcp_servers.*``, or
...
- **"actual" is always bounded *and* redacted before it leaves this
  ...
  ``pipeline.artifact_contracts``'s own ``compliance_report`` "actual"
  ...
  ``mcp_servers.*``/other-``pipeline``-module dependency)
  ``mcp_servers.shared``'s sensitive-key and
  ``pipeline.artifact_contracts``'s tenant-key semantics.
...
def _truncate(value: Any, max_chars: int) -> Any:
    ...
    ``pipeline.artifact_contracts.MAX_COMPLIANCE_MESSAGE_CHARS``) exactly
...
# module's "never imports mcp_servers.*, and never imports any other
# pipeline module" invariant intact and dependency-cycle-free)
# `mcp_servers.shared._is_sensitive_key`'s credential-shaped key set and
# `pipeline.artifact_contracts._is_tenant_key`'s tenant/workspace/account/
...
# (see pipeline/artifact_contracts.py MAX_COMPLIANCE_VALUE_CHARS), or
...
# Comfortably under pipeline.artifact_contracts's own
...
# pipeline.artifact_contracts.build_artifact/write_artifact under the
