...
Problem: ``mcp_servers.shared.redact_sensitive`` irreversibly masks
...
This middleware, enabled only with ``CENTRALMCP_TOKENIZE_SECRETS=1``,
replaces sensitive-keyed string values in tool *results* with an opaque,
session-scoped token (``cmcp_secret_<32 hex chars>``) instead of
...
reachable from the FastMCP ``Context`` passed through by
``mcp_servers._middleware.install`` (stable for the lifetime of one
...
``CENTRALMCP_TOKENIZE_SECRETS`` is unset (the default), every hook here is
...
from mcp_servers.shared import _is_sensitive_key

_ENV_FLAG = "CENTRALMCP_TOKENIZE_SECRETS"
_TOKEN_PREFIX = "cmcp_secret_"
...
class SecretTokenizeMiddleware:
    """Reversible secret/PII tokenization, gated by ``CENTRALMCP_TOKENIZE_SECRETS``.
    ...
    no-op) until the env var is set, so adding it is safe by default::
        ...
        from mcp_servers._middleware import SecretTokenizeMiddleware
    ...
    def __init__(
        ...
        self._opaque_vaults: OrderedDict[int, tuple[Any, _SessionVault]] = OrderedDict()
    ...
    def _vault_for(self, context: Any) -> _SessionVault:
        ...
        try:
            ...
            if entry is None or entry[0] is not session:
                entry = (session, _SessionVault(self.max_entries, self.ttl_seconds))
