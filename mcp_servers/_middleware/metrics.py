...
Set ``CENTRALMCP_METRICS=1`` to enable in-process collection: request
...
- a bounded capability enum (``read``/``write``/``destructive``/
  ``diagnostic``/``unknown``),
- a bounded outcome enum (see ``mcp_servers._middleware._outcome``),
- wall-clock durations, and
- the presence (not the value) of an existing ``truncated: true`` bounding
  marker already emitted by ``mcp_servers.shared.bound_collection_response``
...
a *separate* opt-in gated by ``CENTRALMCP_METRICS_HTTP`` -- see
``mcp_servers.shared.run_server`` -- and is only ever registered alongside
...
class MetricsMiddleware:
    ...
    Inert (every hook a no-op) unless ``CENTRALMCP_METRICS=1`` -- default
    ...
    def __init__(
        ...
        *,
        ...
        self._starts: contextvars.ContextVar[list[float]] = contextvars.ContextVar(
            f"centralmcp_metrics_starts_{id(self)}",
            default=[],
        )
