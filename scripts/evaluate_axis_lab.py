...
1. **Static contract check** (always runs, offline, no network, no
   ...
   ``mcp_servers/axis.py``'s module docstring. This never touches the
   network and always runs, including in CI.
2. **Bounded read-only live check** (opt-in only): when
   ``CENTRALMCP_LIVE_TEST_AXIS_READ=1`` is set *and* ``AXIS_BASE_URL``/
...
3. **Disposable-write plan** (opt-in only, PLAN ONLY -- never executed):
   when ``CENTRALMCP_LIVE_TEST_AXIS_WRITE=1`` is also set, builds a
...
from mcp_servers import axis as axis_tools  # noqa: E402
from mcp_servers.openapi_gen.manifest import load_manifest  # noqa: E402
        ...
        "steps": [
            {
                ...
                "payload": {"name": "centralmcp-lab-placeholder"},
            },
