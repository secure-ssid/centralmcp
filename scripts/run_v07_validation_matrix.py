...
``pipeline.artifact_contracts.VALIDATION_MATRIX_CLASSIFICATIONS``):
...
duplicated) and (b) inspects ``pipeline.live_test_config``'s status API.
...
``scripts/evaluate_axis_lab.py``, ``pipeline/optional_product_evidence.py``),
...
from pipeline import live_test_config  # noqa: E402
...
# pipeline.optional_product_evidence.PERMANENT_OMISSIONS) rather than
...
def _offline_optional_product(platform: str) -> tuple[bool, str]:
    from pipeline import optional_product_evidence as evidence
...
def _classify_platform(platform: str) -> dict[str, Any]:
    ...
    if platform in _PERMANENT_WRITE_GAP_PLATFORMS and write_enabled and credentials_ok:
        return _entry(
            ...
            "are configured (see pipeline.optional_product_evidence.PERMANENT_OMISSIONS)",
