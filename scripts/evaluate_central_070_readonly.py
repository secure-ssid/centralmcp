...
of curated Central read tools (`mcp_servers.monitoring` /
`mcp_servers.config`) against a `FakeCentralClient` stand-in, never touches
the network, and writes a redacted `live_lifecycle_evidence` artifact via
`pipeline.artifact_contracts`.

Two opt-in modes, gated through `pipeline.live_test_config` (never inferred
from credential presence alone):

- `--live-read`: swaps in the real `mcp_servers.shared.get_client()` and
  runs the *same* bounded set of steps as bounded, GET-only calls against a
  live tenant. Requires `CENTRALMCP_LIVE_TEST_CENTRAL_READ=1`. A GET-only
...
- `--live-write`: runs a disposable create/read-back/delete VSF-template
  round trip (`mcp_servers.config.build_vsf_template` /
  `delete_vsf_template`) against a lab-owned scope, then cleans up.
  Requires *both* `CENTRALMCP_LIVE_TEST_CENTRAL_READ=1` and
  `CENTRALMCP_LIVE_TEST_CENTRAL_WRITE=1` (see
  `pipeline.live_test_config.live_test_write_enabled`), plus a `--scope-id`
...
`pipeline.artifact_contracts.write_artifact`, which redacts secrets and
...
from pipeline import live_test_config
...
class FakeCentralClient:
    """Minimal stand-in for `pipeline.clients.central_client.CentralClient`
...
def _live_read_steps() -> list[dict[str, Any]]:
    ...
    from mcp_servers import monitoring as monitoring_tools
...
def run_disposable_write_lifecycle(scope_id: str, device_function: str) -> dict[str, Any]:
    ...
    (`CENTRALMCP_CENTRAL_WRITES=1`, already the default) are set on purpose.
    """
    from mcp_servers import config as config_tools

    name = f"centralmcp-lab-vsf-{uuid.uuid4().hex[:8]}"
    steps: list[dict[str, Any]] = []
