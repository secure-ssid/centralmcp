...
each platform (see ``pipeline/optional_product_evidence.py``). Additionally
builds a bounded, read-only live-evidence artifact for a platform only when
``CENTRALMCP_LIVE_TEST_<PLATFORM>_READ=1`` is set *and* that platform's
credentials are configured (see ``pipeline/live_test_config.py``) -- neither
...
from pipeline import optional_product_evidence as evidence  # noqa: E402
