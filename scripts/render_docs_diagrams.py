...
ACCESSIBILITY = {
    ...
    "quickstart-journey": (
        "centralmcp quickstart journey",
        "Six steps from cloning centralmcp through setup, doctor checks, MCP connection, tool discovery, and a safe read-only call.",
    ),
    "router-safety-flow": (
    ...
    "troubleshooting-tree": (
        "centralmcp troubleshooting decision tree",
    ...
    "transport-deployment": (
        ...
        "Deployment paths for stdio, local streamable HTTP, and protected non-loopback streamable HTTP connections to the centralmcp router.",
    ),
}
...
def render() -> None:
    ...
    for source in sorted(SOURCE_DIR.glob("*.mmd")):
        ...
        with tempfile.TemporaryDirectory(prefix="centralmcp-mermaid-") as temp_dir:
