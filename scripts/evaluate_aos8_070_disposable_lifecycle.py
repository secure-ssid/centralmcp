...
Every mode is gated through `pipeline.live_test_config` (platform
...
- `--mode status` never makes a network call. It reports the same redacted
  ...
  authorization.
- `--mode read` requires `CENTRALMCP_LIVE_TEST_CENTRAL_READ=1`
...
- `--mode write` requires *both* the read opt-in and
  `CENTRALMCP_LIVE_TEST_CENTRAL_WRITE=1`
...
corresponding `pipeline.live_test_config` opt-in(s) are set. Per the
...
from pipeline.live_test_config import (
...
PLATFORM = "central"
DEFAULT_LAB_PREFIX = "centralmcp-lab-"
...
class LifecycleKind:
    ...
    candidate dict in the exact shape `pipeline.aos8_migration.build_migration_plan`
    ...
    # (`pipeline.aos8_migration`) identifier convention encodes extra
...
def read_evidence(
    ...
    *,
    ...
    scope_name: str,
    persona: str,
    lab_name: str = "centralmcp-lab-probe",
) -> dict[str, Any]:
    ...
    if not live_test_read_enabled(PLATFORM):
        raise LifecycleHarnessError(
            ...
            "CENTRALMCP_LIVE_TEST_CENTRAL_READ=1 to enable."
        )
...
def run_disposable_write_lifecycle(
    ...
    *,
    ...
    requires the read opt-in -- see `pipeline.live_test_config`), `confirm`,
    ...
    if not live_test_write_enabled(PLATFORM):
        raise LifecycleHarnessError(
            ...
            "CENTRALMCP_LIVE_TEST_CENTRAL_READ=1 and "
            "CENTRALMCP_LIVE_TEST_CENTRAL_WRITE=1 to enable."
        )
    if len(lab_prefix) < 6:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # -- doing so requires the production `mcp_servers.aos8` bindings
