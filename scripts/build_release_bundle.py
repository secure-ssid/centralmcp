#!/usr/bin/env python3
"""Build one deterministic release-artifacts bundle (v0.7 live-artifacts).
...
* ``pipeline.optional_product_evidence`` -- optional-backend compatibility evidence
...
* ``pipeline.release_packaging`` -- provenance/checksums/deterministic archive primitives
* ``pipeline.sbom`` -- CycloneDX 1.5 JSON SBOM from ``uv.lock``
...
Produces, under ``--output-dir`` (default ``dist/``):
    ...
    <output-dir>/<version>/
        ...
        indexes/
            centralmcp-rag-index-<version>.tar.gz(.sha256)   (only if data/ indexes are present)
    ...
    <output-dir>/centralmcp-release-artifacts-<version>.tar.gz(.sha256)
...
from pipeline import release_packaging  # noqa: E402
from pipeline import sbom as sbom_module  # noqa: E402
...
def assemble_release_bundle(
    *,
    ...
    from pipeline import optional_product_evidence
