...
only calls `mcp_servers.monitoring.get_global_scope_id`,
`mcp_servers.monitoring.list_scopes`, and
`mcp_servers.aos8.aos8_preview_migration_run`. A GET-only guard is installed on
`mcp_servers.shared.get_client()._request` before any live call so a non-GET
...
from mcp_servers import aos8 as aos8_tools
from mcp_servers import monitoring as monitoring_tools
from mcp_servers import shared as shared_tools
