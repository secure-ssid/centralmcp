...
``pipeline/artifact_contracts.py``) and exits non-zero if any source is
...
def _entry(source: str, count: int, minimum: int, status: str, detail: str = "") -> dict[str, Any]:
    return {
        "source": source,
...
def _evaluate(
    *,
    ...
    try:
        parsed = parse(raw)
    except (sources.SourceFetchError, provenance.SourceProvenanceError) as exc:
        return _entry(source, 0, minimum, STATUS_CHANGED, str(exc))
...
def _changed_from(entry: dict[str, Any], exc: Exception) -> dict[str, Any]:
    ...
    return _entry(entry["source"], entry["count"], entry["minimum"], STATUS_CHANGED, str(exc))
...
def main() -> int:
    ...
    for entry in entries:
        ...
        print(f"{entry['source']}: {entry['status']} ({entry['count']}/{entry['minimum']}){detail}")
    ...
    if failing:
        detail = "; ".join(f"{entry['source']}={entry['status']}" for entry in failing)
