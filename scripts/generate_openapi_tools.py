...
ingestion/sources/); only the compact, license-clean manifest is written to
mcp_servers/openapi_gen/manifests/<platform>.json.
"""
...
from mcp_servers.openapi_gen import manifest as manifest_mod  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument(
        "--spec",
        default=None,
        help="path to the OpenAPI spec (defaults to the known platform location)",
    )
    ap.add_argument(
        "--check",
        action="store_true",
    ...
    spec_arg = args.spec or _DEFAULT_SPECS.get(args.platform)
    if not spec_arg:
    ...
    manifest = build(args.platform, spec_path)
