...
`mcp_servers.tool_router.plan_tool_workflow` and
`mcp_servers.tool_router.plan_reconciliation_schedule` -- against the
currently enabled backend catalog (whatever `CENTRALMCP_TOOLSETS` /
`CENTRALMCP_PRODUCTS` resolve to in the running environment; the base
    ...
    if plan_fn is None:
        print(
            json.dumps(
                {
                    "status": "skipped",
                    "reason": "plan_tool_workflow not registered (CENTRALMCP_ROUTER_MODE=minimal)",
                },
                indent=2,
            )
        )
