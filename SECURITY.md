...
- Central and optional platform writes can be disabled independently with
  `CENTRALMCP_<PLATFORM>_WRITES`.
- Optional products default to `CENTRALMCP_PRODUCT_ACCESS=read-only`; guarded
...
- `CENTRALMCP_TOKENIZE_SECRETS=1` enables bounded session-scoped secret
